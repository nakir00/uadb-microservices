...
import { MaisonColumns } from '@/blocs/proprietaire/table/columns/maison-columns'
...
function RouteComponent() {
  const proprietaireId = Route.useLoaderData().proprietaireId!
  const maisonCreate = api.maison.create()
  const [sortBy, setSortBy] = useState<keyof MaisonModel>('id')
  const [page, setPage] = useState(0)
  const [size, setSize] = useState(10)
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc')
  const [search, setSearch] = useState<string | undefined>(undefined)
  const { isPending, data, refetch } = api.maison.getAll({
    page,
    size,
    proprietaireId,
  ...
  useEffect(() => {
    subscribe('refresh_maison_table', (e) => {
      refetch()
    })

    return () => {
      unsubscribe('refresh_maison_table', () => {})
    }
  }, [])
    ...
    <div className="flex flex-1 flex-col gap-4 lg:gap-6 py-4 lg:py-6 ">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold">Maisons</h1>
          <p className="text-sm text-muted-foreground">
            liste des maison que vous gérer
          </p>
        </div>
        <CreateMaisonButton onCreateMaison={ () => refetch()} />
      </div>
      {/* Numbers */}
      <StatsGrid
        stats={[
      ...
      <div className="min-h-[100vh] flex-1 md:min-h-min">
        <NormalDataTable
          ...
          data={data?.data.content as Array<MaisonModel>} // {maisonRead.data?.content as Array<MaisonModel>}
          onDelete={() => {
            console.log('deleted')
          }}
          isLoading={isPending}
